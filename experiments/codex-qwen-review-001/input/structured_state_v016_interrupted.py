...
def reconcile_structured_state(
    ...
    *,
    ...
    reconciled = merge_structured_states(cached, document_state)
...
def rebase_conflicts(
    ...
    for category, (id_field, _prefix, _semantic_fields) in CATEGORY_SPECS.items():
        ...
        for incoming in safe_delta.get(category, []) or []:
            ...
            if not identifier or identifier not in base_items:
                continue
            if current_items.get(identifier) != base_items.get(identifier):
