# QH-V2-SPEC-001B - Create REQUIREMENTS.md From Exact Artifact Content
...
Copy the text between `BEGIN_ARTIFACT` and `END_ARTIFACT` into `REQUIREMENTS.md`.
Do not copy any text outside that block into `REQUIREMENTS.md`.
...
- Its content exactly matches the text between `BEGIN_ARTIFACT` and `END_ARTIFACT`.
- No Task-control sections are copied into `REQUIREMENTS.md`.
- No other Repository file is modified.

## Stop Condition

Stop immediately after creating `REQUIREMENTS.md`.
